...
export default memo(function Composer({refresh,notify}){
 ...
 <textarea ref={editor} aria-label="Text to share" placeholder="Paste…" value={text} onChange={e=>setText(e.target.value)} maxLength={10485760} onKeyDown={e=>{if(e.key==='Enter'&&(e.ctrlKey||e.metaKey)){e.preventDefault();send();}}}/>
 <div className="flex items-center justify-between composer-toolbar"><div className="flex items-center gap-2"><IconButton label="Attach file" disabled={busy} onClick={()=>input.current.click()}><Paperclip/></IconButton><IconButton label="Paste" onClick={paste}><Clipboard/></IconButton><input ref={input} type="file" hidden onChange={e=>upload(e.target.files[0])}/></div><IconButton label={sent?'Sent':'Send'} type="submit" className="send-button" disabled={busy||!text.trim()}>{busy?<LoaderCircle className="spin"/>:sent?<Check className="confirm"/>:<ArrowUp/>}</IconButton></div>
